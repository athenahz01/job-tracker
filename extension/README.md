...
The popup saves the current job page with one click. If the company cannot be found, it asks for the company before posting.

Greenhouse, Lever, Ashby, and Workday content scripts look for a submitted application confirmation before posting automatically. Generic submitted-page detection only runs from the popup through `activeTab`, so the extension does not need broad host access for every site.
